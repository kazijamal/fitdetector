...
> = ({ authSession }) => {
  ...
  const {
    ...
    isLoading,
    error,
  } = api.celebrity.getById.useQuery({ id });

  const followMutation = api.celebrity.follow.useMutation({
    onSuccess: (data) => {
      utils.celebrity.getById.invalidate();
    },
  });

  const unfollowMutation = api.celebrity.unfollow.useMutation({
    onSuccess: (data) => {
      utils.celebrity.getById.invalidate();
    },
  });
